...
- **Geração automática de código Cypress**: Transforma ações em comandos como `cy.get('seletor').click();` ou `cy.get('seletor').type('texto');`.
  ...
  - `cy.get('.price-value').should('have.text', 'R$ 10,90');`
...
🎬 Iniciar Gravação    🛑 Parar Gravação    ⬇️ Exportar Código

┌─────────────────────────────────────────┐
│ Nome do arquivo:                        │
│ [Digite o nome do arquivo (ex: login..)]│
│ A extensão .js será adicionada automaticamente │
└─────────────────────────────────────────┘

// Código Cypress será exibido aqui... 
...
cy.get('#user-avatar').should('have.attr', 'src', '/avatar.jpg');
cy.get('.price-value').should('have.text', '10,90');
cy.get('#login-btn').should('be.visible');
cy.get('[name="email"]').should('have.value', 'teste@teste.com'); 